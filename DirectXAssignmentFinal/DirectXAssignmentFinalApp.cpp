...
void DirectXAssignmentFinalApp::BuildBoxGeometry()
{
	...
	{
		auto& p = hexagon.Vertices[i].Position;
...
void DirectXAssignmentFinalApp::BuildMaterials()
{
	...
	sandBrick->MatCBIndex = 8;
	sandBrick->DiffuseSrvHeapIndex = 7;
	sandBrick->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	sandBrick->FresnelR0 = XMFLOAT3(0.02f, 0.02f, 0.02f);
	sandBrick->Roughness = 0.25f;

	auto mossy = std::make_unique<Material>();
	mossy->Name = "mossy";
	mossy->MatCBIndex = cbIndex++;
	mossy->DiffuseSrvHeapIndex = srvHeapIndex++;
	...
	stonestep->MatCBIndex = cbIndex++;
	stonestep->DiffuseSrvHeapIndex = srvHeapIndex++;
...
void DirectXAssignmentFinalApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&rightCastleWall->World, XMMatrixScaling(12.0f, 30.0f, 20.0f)*XMMatrixTranslation(15.0f, yLevel + 7.5f, 0.0f));
	...
	XMStoreFloat4x4(&frontLeftCastleWall->World, XMMatrixScaling(7.0f, 24.0f, 2.0f)*XMMatrixTranslation(-10.0f, yLevel + 6.0f, -15.5f));
	...
	XMStoreFloat4x4(&frontRightCastleWall->World, XMMatrixScaling(7.0f, 24.0f, 2.0f)*XMMatrixTranslation(10.0f, yLevel + 6.0f, -15.5f));
	...
	XMStoreFloat4x4(&frontRightCastlePillar->World, XMMatrixScaling(2.0f, 40.0f, 2.0f)*XMMatrixTranslation(15.0f, yLevel + 8.5f, -15.5f));
	...
	XMStoreFloat4x4(&frontLeftCastlePillar->World, XMMatrixScaling(2.0f, 40.0f, 2.0f)*XMMatrixTranslation(-15.1f, yLevel + 8.5f, -15.6f));
